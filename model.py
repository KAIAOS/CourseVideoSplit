...
def text_detect(img):
    _, _, rects_re, _ = text_handle.predict(img, long_size=pse_long_size)
    return rects_re


def text_rec(img, rects) -> list:
    """
    :param img: cv2 img in BGR Color
    :param rects: text boxes in img
    :return: Title and All text
    """
    res = crnnRec(np.array(img), rects)
    return res
...
def text_predict(img):
    ...
    cv2.imwrite("/home/henrylee/draw.jpg", img2)
...
if __name__ == "__main__":
    # img = cv2.imread('/home/henrylee/course_videos/HUST-1003157001/[第4章 电路定理] 第1讲 叠加定理.mp4')
    #img = cv2.imread('/home/henrylee/remote/cpu-node3/course_pic/tmp.jpg')
    ...
    img2 = draw_bbox(img, boxes_list, color=(0, 255, 0))
    result = text_rec(img, rects_re)

    print("time: ", (datetime.datetime.now() - t1))
    for r in result:
        print(r)
    #cv2.imwrite("/home/henrylee/remote/cpu-node3/tmp_out.png", img2)
    cv2.imwrite("tmp_out.png", img2)
    pass
